...
class DaftExprtDataLoader(Dataset):
    ...
    '''
    def __init__(self, data_file, hparams, shuffle=True, emotion=None):
        ...
        if emotion is None:
            ...
            self.data = self.data = [line.strip().split(sep='|') for line in lines if line.strip().split(sep='|')[3] == emotion]
        # self.data = [line.strip().split(sep='|') for line in lines]
    ...
    def get_data(self, data):
        ...
        return symbols, durations_float, durations_int, symbols_energy, symbols_pitch, \
            frames_energy, frames_pitch, mel_spec, speaker_id, features_dir, feature_file, emotion
        # return symbols, durations_float, durations_int, symbols_energy, symbols_pitch, \
...
class DaftExprtDataCollate():
    ...
    def __call__(self, batch):
        ...
        feature_dirs, feature_files, emotions = [], [], []
        for i in range(len(ids_sorted_decreasing)):
        ...
        return symbols, durations_float, durations_int, symbols_energy, symbols_pitch, input_lengths, \
            frames_energy, frames_pitch, mel_specs, output_lengths, speaker_ids, feature_dirs, feature_files, emotions


def prepare_data_loaders(hparams, num_workers=1, drop_last=True, emotion=False, emotions=[]):
    ...
    if emotion:
        # emotion_set = DaftExprtDataLoader(hparams.training_files, hparams)
        for e in emotions:
            emotion_set[e] = DaftExprtDataLoader(hparams.training_files, hparams, emotion=e)
    if hparams.contrastive_type == 'self-supervised':
        ss_set = DaftExprtDataLoader(hparams.training_files, hparams)
        ss_loader = DataLoader(ss_set, num_workers=num_workers, shuffle=True,
                              batch_size=hparams.batch_size*8, pin_memory=True, drop_last=drop_last, collate_fn=collate_fn)
    ...
    val_loader = DataLoader(val_set, num_workers=num_workers, shuffle=False, 
                            batch_size=hparams.batch_size, pin_memory=True, drop_last=False, collate_fn=collate_fn)
    emotion_loader = {}
    if emotion:
        for e in emotions:
            emotion_loader[e] = DataLoader(emotion_set[e], num_workers=num_workers, shuffle=True, 
                                           batch_size=hparams.batch_size, pin_memory=True, drop_last=True, collate_fn=collate_fn)
    # emotion_loader = DataLoader(emotion_set, num_workers=num_workers, shuffle=True,
    #                           batch_size=hparams.batch_size, pin_memory=True, drop_last=drop_last, collate_fn=collate_fn)
    
    return train_loader, train_sampler, val_loader, emotion_loader, ss_loader, nb_training_examples
