...
class PositionWiseConvFF(nn.Module):
    ...
    def forward(self, x, film_params):
        ...
        # add residual connection and perform layer normalization
        no_film_outputs = outputs = self.layer_norm(outputs + x)  # (B, L_max, hidden_embed_dim)
        ...
        return no_film_outputs, outputs
...
class FFTBlock(nn.Module):
    ...
    def forward(self, x, film_params, mask):
        ...
        # feed-forward pass
        no_film_outputs, outputs = self.feed_forward(attn_outputs, film_params)  # (B, L_max, hidden_embed_dim)
        outputs = outputs.masked_fill(mask.unsqueeze(2), 0)  # (B, L_max, hidden_embed_dim)
        no_film_outputs = no_film_outputs.masked_fill(mask.unsqueeze(2), 0)
...
class EmotionClassifier(nn.Module):
    ...
    def __init__(self, hparams):
        ...
        nb_speakers = len(hparams.emotions)
        embed_dim = hparams.emotion_classifier['hidden_embed_dim']
...
class ProsodyEncoder(nn.Module):
    ...
    def forward(self, frames_energy, frames_pitch, mel_specs, speaker_ids, output_lengths):
        ...
        for _, block in enumerate(self.blocks):
            _, outputs = block(outputs, None, mask)  # (B, T_max, hidden_embed_dim)
...
class PhonemeEncoder(nn.Module):
    ...
    def forward(self, x, film_params, input_lengths):
        ...
        # no_film_x = x = x.masked_fill(mask.unsqueeze(2), 0)  # (B, L_max, hidden_embed_dim)
        no_film_x = x = x.masked_fill(mask.unsqueeze(2), 0)  # (B, L_max, hidden_embed_dim)
        # pass through FFT blocks
        for idx, block in enumerate(self.blocks):
            no_film_x, x = block(x, film_params[:, idx, :], mask)  # (B, L_max, hidden_embed_dim)
            # no_film_x, b = block(no_film_x, film_params[:, idx, :], mask)
            # print("hehe")
...
class FrameDecoder(nn.Module):
    ...
    def forward(self, x, film_params, output_lengths):
        ...
        # pass through FFT blocks
        for idx, block in enumerate(self.blocks):
            _, x = block(x, film_params[:, idx, :], mask)  # (B, T_max, hidden_embed_dim)
...
class DaftExprt(nn.Module):
    ...
    def __init__(self, hparams):
        ...
        if hparams.use_emotion_classifier:
            self.emotion_classifier = EmotionClassifier(hparams)
            self.mapping = {emotion: i for i, emotion in enumerate(hparams.emotions)}

    def parse_batch(self, gpu, batch, emotion=False):
        ''' Parse input batch
        '''
        # extract tensors
        if emotion or self.use_emotion_classifier:
            symbols, durations_float, durations_int, symbols_energy, symbols_pitch, input_lengths, \
                frames_energy, frames_pitch, mel_specs, output_lengths, speaker_ids, feature_dirs, feature_files, emotion = batch
        else: 
            symbols, durations_float, durations_int, symbols_energy, symbols_pitch, input_lengths, \
                frames_energy, frames_pitch, mel_specs, output_lengths, speaker_ids, feature_dirs, feature_files, _ = batch
            emotion = None
        
        mapped_emotion = None
        if self.use_emotion_classifier:
            mapped_emotion = [self.mapping[emotion] for emotion in emotion]
            mapped_emotion = torch.as_tensor(mapped_emotion)
            mapped_emotion = mapped_emotion.cuda(gpu, non_blocking=True).long()
        ...
        inputs = (symbols, durations_float, durations_int, symbols_energy, symbols_pitch, input_lengths,
                  frames_energy, frames_pitch, mel_specs, output_lengths, speaker_ids, emotion, mapped_emotion)
        targets = (durations_float, symbols_energy, symbols_pitch, mel_specs, speaker_ids, emotion, mapped_emotion)
    ...
    def forward(self, inputs, emo=False):
        ''' Forward function of DaftExprt
        '''
        # extract inputs
        if emo:
            frames_energy, frames_pitch, mel_specs, output_lengths, speaker_ids, emotion, mapped_emotion = inputs
        ...
        emo_preds = None
        if self.use_emotion_classifier:
            emo_preds = self.emotion_classifier(prosody_embed)  # (B, nb_speakers)
        # embed phoneme symbols, add positional encoding and encode input sequence
        enc_outputs_no_film, enc_outputs = self.phoneme_encoder(symbols, encoder_film, input_lengths)  # (B, L_max, hidden_embed_dim)
        ...
        alignments = weights
        mi = [enc_outputs_no_film, prosody_embed]
        
        return speaker_preds, film_params, encoder_preds, decoder_preds, alignments, mi, prosody_embed, emotion, emo_preds
        # return speaker_preds, film_params, encoder_preds, decoder_preds, alignments, mi
    ...
    def inference(self, inputs, pitch_transform, hparams):
        ...
        # embed phoneme symbols, add positional encoding and encode input sequence
        _, enc_outputs = self.phoneme_encoder(symbols, encoder_film, input_lengths)  # (B, L_max, hidden_embed_dim)
        # predict prosody parameters
        duration_preds, energy_preds, pitch_preds = self.prosody_predictor(enc_outputs, prosody_pred_film, input_lengths)  # (B, L_max)
...
#         # print("y_samples size:", y_samples.size())
#         # print("x_samples size:", y_samples.size())
#         # print("logvar size: ", logvar.exp().size())
...
#         self.lstm = nn.LSTM(x_dim, x_dim, num_layers=1, batch_first=True)
#         self.sigmoid = nn.Linear(x_dim, x_dim)
#         self.layer_norm = nn.LayerNorm(x_dim)
...
#     def mi_est(self, x_samples, y_samples):
#         lstm_out, _ = self.lstm(x_samples)
#         lstm_out = lstm_out[:, -1, :]
#         x_samples = self.sigmoid(lstm_out)
...
#         final = self.layer_norm(lstm_out)
        
        
#         mu, logvar = self.get_mu_logvar(final)
