...
def mi_forward(mi_model, phoneme, prosody, mi_optimizer):
        ...
        # lld_loss = -mi_model.loglikeli(prosody, prosody)
        lld_loss.backward()
        mi_optimizer.step()
        _logger.info(f'Train loss [{lld_loss:.6f}]')
        return mi_model, mi_optimizer, lld_loss

def normalize_columns(tensor):
    # Transpose the tensor to work on columns
    transposed_tensor = tensor.t()

    # Normalize each column
    normalized_tensor = (transposed_tensor - transposed_tensor.mean(dim=0)) / transposed_tensor.std(dim=0)

    # Transpose the normalized tensor back to its original shape
...
def train(gpu, hparams, log_file):
    ...
    mi_optimizer = Adam(mi_model.parameters(), lr=5e-3)
    ...
    train_loader, train_sampler, val_loader, neutral_loader, angry_loader, happy_loader, sad_loader, surprise_loader ,nb_training_examples = \
    ...
    indiv_loss = {
        ...
        'duration_loss': 0., 'energy_loss':0., 'pitch_loss': 0.,
        'mel_spec_l1_loss': 0., 'mel_spec_l2_loss': 0., 'mi_loss': 0.,
        'mi_predictor_loss': 0.
    }
    total_time = 0.
    start = time.time()
    ...
    for epoch in range(epoch_offset, epochs):
        ...
        for batch in train_loader:
            ...
            mi_predictor_loss = 0.
            mi_loop = 5
            # d_phoneme_embed = normalize_columns(d_phoneme_embed)
            # d_prosody_embed = normalize_columns(d_prosody_embed)
